...
<template>
    ...
    <v-container v-else fluid pa-0 ma-0 style="height:90vh;" class="events-container">
        ...
        <v-flex xs8 style="position: relative; padding: 0px 65px 0;">
            <v-btn style="position:absolute; top:0; right:64px; z-index: 2020;" dark small @click="videoShowing = !videoShowing">{{ videoShowing ? 'SHOW IMAGE' : 'PLAY VIDEO' }}</v-btn>
...
export default {
    components: {
        ...
        'active-list-item': ActiveListItem,
        ...
        async updateEvent(event) {
            ...
            try {
                ...
                if (response.status && response.status === 500) {
                    this.$notifyError("ERROR GETTING TODAYS EVENTS. PLEASE TRY AGAIN LATER");
                    return;
                }
                this.$notifyClassification(event.user_classification.toUpperCase());
