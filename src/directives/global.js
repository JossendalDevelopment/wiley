export const trim = {
    bind(el, binding) {
        // this directive accepts a value with with to trim a string and appends '...'
        if(!binding.value) {
            throw new Error("Custom directive 'v-trim' is missing a required value.");
        }
...
export const testref = {
    bind(el, binding) {
        ...
        if(!binding.value) {
            throw new Error("Custom directive 'v-test-ref' is missing a required value.");
        }
        if (el) {
